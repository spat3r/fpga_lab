...
// TODO: Az együtthatók s7.8, ennek megfelelően kell trimmelni a DSP kimenetet.
	...
	temp = getchar();
	if ( (temp == *LF) || (temp == *CR) ) { i--; continue; }
	else if ( temp == *neg_sign ) { sign = -1; continue; }
	else if ( temp == *pos_sign ) { sign =  1; continue; }
		...
		while(coeff_read_finish) {
			...
			switch (state) {
				case 0:
					...
					else if ( (temp == *period) ) { state++; continue; }
					// else { error }
					continue;
				...
				case 1:
					...
					else if ( temp == *neg_sign ) { sign = -1; state = 0; }
					else if ( temp == *pos_sign ) { sign =  1; state = 0; }
					// else { error }
					continue;
			...
			MEM32(FIR_ADDR + (j<<2) ) = coeff & 0x00FF;
		}
	}
